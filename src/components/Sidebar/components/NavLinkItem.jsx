...
export default function NavLinkItem({ item }) {
  // props 목록 간소화
  const location = useLocation(); // 🔴 Zustand에서 필요한 상태와 액션을 구독 🔴

  const { activeItem, handleItemClick, handleSubMenuClick } = useSidebarStore(); // 1. 활성화 상태 계산 (배경색 결정):
  ...
  const isActive = isLabelActive || isSubMenuActive; // 2. 시각적 활성화 상태 계산 (텍스트/아이콘 색상 결정):
  ...
  const props = isNavigational
    ...
    : {
        ...
        role: 'button',
      }; // 💡 [수정] 오타 수정: '카테고리'를 '챌린지'로 변경 (sidebarData에서 '챌린지'로 레이블을 제공한다고 가정)
